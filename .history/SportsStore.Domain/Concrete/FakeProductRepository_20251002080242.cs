...
public IAsQueryable(Product) Products=>products.AsQueryable();
...
public void SaveProduct(Product product)
{
    ...
    if(product.ProductID==0){
        ...
        //Nếu ID(SẢn Phẩm) đã tồn tại . p=>p.ProductID==ProductID cũng được luôn
        Product? existingProduct=products.FirstOrDefault(p=>p.ProductID==product.ProductID);
        if(existingProduct!=null){
            //cập nhật dữ liệu 
            existingProduct.Name=product.Name=product.Name;
